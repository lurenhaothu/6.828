...
sys_getenvid(void)
{
	//cprintf("called getenvid, the id is %x\n", curenv->env_id);
...
sys_exofork(void)
{
	...
	struct Env* e;
	int error_code = env_alloc(&e, thiscpu->cpu_env->env_id);
	if(error_code == 0){
...
sys_env_set_status(envid_t envid, int status)
{
	...
	if(error_code == -E_BAD_ENV) return -E_BAD_ENV;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pte_t* pte_entry;
	struct PageInfo *pp = page_lookup(srce->env_pgdir, srcva, &pte_entry);
	if(pp == NULL) return -E_INVAL;
	if(!(perm & PTE_U) | !(perm & PTE_P)) return -E_INVAL;
	if((perm & ~(PTE_U | PTE_P | PTE_AVAIL | PTE_W)) != 0) return -E_INVAL;
	if(((*pte_entry & PTE_W) == 0) && ((perm & PTE_W) != 0)) return -E_INVAL;
	error_code = page_insert(dste->env_pgdir, pp, dstva, perm);
	if(error_code == -E_NO_MEM) return -E_NO_MEM;
	else return 0;
}
